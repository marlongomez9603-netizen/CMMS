...
| Tramo | Actividad |
|---|---|
| 0:00–0:20 | **Demo en vivo**: registra TÚ la Bomba (BOM-001) + 1 plan + muestra el calendario y el PDF. |
...
| 2:10–2:40 | Distribuyen fechas, revisan **Calendario** y **descargan los PDF**. |
| 2:40–3:00 | Cierre: revisas el **panel docente en tiempo real** y comentas resultados. |

> Antes de iniciar: confirma que cada estudiante entra con su cédula y que ves su empresa/planta en el **panel docente**.
...
- Dejar **Próxima Ejecución** vacía → no aparece en el calendario.
- Criticidad por defecto "Alta" en el motor (debería ser Media).

---

## Variantes (para evitar copia)
- Puedes pedir que **cambien la marca/modelo** por equipos reales que ellos investiguen.
- O asignar criticidades distintas por grupo y que **justifiquen** con una matriz de criticidad (consecuencia × frecuencia).
- O inyectarles una **avería** (botón ⚡ del panel docente) a mitad de sesión para que además gestionen la **OT correctiva con RCA**.

---

## Inyección de avería (extensión opcional, +30 min)
1. Desde el **panel docente** → fila del estudiante → botón ⚡ → elige un equipo y prioridad crítica.
2. El estudiante recibe la alerta → debe ir a **Órdenes de Trabajo** → crear la **OT correctiva** → hacer el **RCA (5 porqués)** → cerrarla.
3. Verifica en tiempo real cómo cambian su **disponibilidad** y **costo**.
